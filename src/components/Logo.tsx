...
export default function Logo({ className = '', shieldSize = 44, withText = true }: LogoProps) {
  // We reference /logo.png as the primary source. If the file is not yet uploaded or fails to load,
  // we gracefully fall back to our pixel-perfect, hand-crafted vector SVG shield illustration.
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      {!imgFailed ? (
        <img
          src="/logo.png"
          alt="Euro Second Opinion Logo"
          onError={() => setImgFailed(true)}
          style={{ width: shieldSize, height: 'auto' }}
          className="shrink-0 drop-shadow-md select-none object-contain"
